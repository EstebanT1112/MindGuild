...
export default function NewQuestionModal({
  ...
  const toggleTopic = (topicId: string) => {
    setSelectedTopicIds((current) => {
      ...
      if (current.length >= 5) {
        setErrorMessage('Podés seleccionar hasta 5 temas por pregunta.');
        return current;
      }
  ...
  const handleCreateTopic = async () => {
    ...
    const existingTopic = topics.find(
      (topic) => normalizeTopicName(topic.name) === normalizeTopicName(name)
    );
    if (existingTopic) {
      setSelectedTopicIds((current) =>
        current.includes(existingTopic.id) ? current : [...current, existingTopic.id].slice(0, 5)
      );
      setNewTopicName('');
    ...
    try {
      ...
      setSelectedTopicIds((current) =>
        current.includes(topic.id) ? current : [...current, topic.id].slice(0, 5)
      );
      setNewTopicName('');
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmedQuestion) {
      setErrorMessage('Escribí el enunciado de la pregunta.');
      return;
    }
    ...
    try {
      ...
      setErrorMessage(error.message ?? 'Revisá los datos e intentá nuevamente.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.content}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.typeRow}>
              ...
              >
                <Text
                  style={[styles.typeText, type === 'multiple_choice' && styles.typeTextActive]}
                >
                  Multiple Choice
                </Text>
              </Pressable>
            ...
            <View style={styles.topicSection}>
              <Text style={styles.label}>Tema académico</Text>
              {topicsLoading ? (
                <ActivityIndicator color={colors.cyan} style={styles.topicLoader} />
              ) : topics.length === 0 ? (
                <Text style={styles.hint}>
                  Sin temas creados. Podés crear uno o dejar la pregunta sin clasificar.
                </Text>
              ) : (
                <View style={styles.topicList}>
                  {topics.map((topic) => {
                    ...
                    return (
                      ...
                      >
                        <Text
                          style={[styles.topicText, selected && styles.topicTextActive]}
                        >
                          {topic.name}
                        </Text>
                      </Pressable>
            ...
            {type === 'multiple_choice' ? (
              <View style={{ marginTop: 15 }}>
                ...
                {options.map((option, index) => (
                  ...
                  >
                    <View
                      style={[styles.radio, selectedOption === index && styles.radioActive]}
                    >
                      {selectedOption === index && <Check color="#ffffff" size={12} />}
                    </View>
                    <TextInput
                ...
                <Text style={styles.hint}>
                  Seleccioná exactamente una respuesta correcta.
                </Text>
              </View>
            ) : (
              <View style={{ marginTop: 15 }}>
                ...
                <TextInput
                  ...
                  placeholder="Escribí la respuesta esperada..."
                  placeholderTextColor={colors.textMuted}
                  multiline
...
const createStyles = (colors: any) =>
  StyleSheet.create({
    ...
    typeBtnActive: {
      ...
      borderColor: colors.text, // 👈 Borde oscuro en claro, claro en oscuro
    },
    typeText: {
      color: colors.textMuted,
      fontWeight: 'bold',
    ...
    typeTextActive: {
      color: colors.text, // 👈 Se adapta al tema
    },
    input: {
    ...
    newTopicBtn: {
      width: 44,
      borderRadius: 12,
      backgroundColor: colors.input, // 👈 Mismo fondo que el input
      borderWidth: 1,
      borderColor: colors.inputBorder,
      alignItems: 'center',
      justifyContent: 'center',
